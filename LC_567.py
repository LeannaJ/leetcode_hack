...
def contains_permutation(s1: str, s2: str, normalize: bool = True) -> bool:
    # Optional normalization for messy inputs in live coding platforms
    # 1) strip spaces  2) lowercasing
    # NOTE: Comment out if the problem guarantees clean lowercase inputs.
    if normalize:
    ...
    # Edge cases
    if not s1:  # empty pattern: define as True iff we consider empty permutation exists
        # For LC 567 specifically, s1 is non-empty by constraints, but we guard anyway.
        return True
    if len(s1) > len(s2):
        return False
    
    k = len(s1)
    need = Counter(s1)       # frequency map for s1 (works for any unicode/char set)
    window = Counter(s2[:k]) # frequency map for the first window of s2
    
    # If initial window matches, success
    if window == need:
        return True
    
    # Slide window over s2
    for i in range(k, len(s2)):
        in_ch = s2[i]         # new char entering
        out_ch = s2[i - k]    # old char leaving
        ...
        window[out_ch] -= 1
        if window[out_ch] == 0:
            # Keep counter small; removing zero entries helps equality checks be faster
            del window[out_ch]
...
if __name__ == "__main__":
    # Example I/O handling (CoderPad/HackerRank friendly)
    ...
    # Run with normalization on (toggle to False if inputs are guaranteed lowercase a-z)
    ans = contains_permutation(s1, s2, normalize=True)
